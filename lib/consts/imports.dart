// packages
export 'package:flutter/material.dart';
export 'package:get/get.dart';
export 'package:velocity_x/velocity_x.dart';
export 'package:firebase_core/firebase_core.dart';
export 'package:firebase_auth/firebase_auth.dart';
export 'package:fluttertoast/fluttertoast.dart';
export 'package:simple_fontellico_progress_dialog/simple_fontico_loading.dart';
export 'package:shared_preferences/shared_preferences.dart';
export 'package:cloud_firestore/cloud_firestore.dart';

// screens
export '../../../screens/splash/splash_screen.dart';
export '../../../screens/onboarding/onboarding_screen.dart';
export '../../../screens/signup/signup_page.dart';
export '../../../screens/login/login_page.dart';

// consts
export '../../consts/images.dart';
export '../../consts/strings.dart';
export '../../consts/fonts.dart';
export '../../consts/colors.dart';
export '../../consts/validation.dart';
export '../../consts/shared_prefs.dart';

// widgets
export '../../widgets/elevatedButton_widgets.dart';
export '../../widgets/textformfields_widgets.dart';
export '../../widgets/snackbar.dart';
export '../../widgets/toast.dart';
export '../../widgets/loader.dart';

// auth
export '../../auth/auth_service.dart';

// bottom nav
export '../../../screens/bottomnav/bottom_nav_page.dart';

// pages
export '../../../screens/pages/home_page.dart';
export '../../../screens/pages/work_in_progress_page.dart';
export '../../../screens/pages/course_page.dart';


// controllers
export '../../../controller/bottom_nav_controller.dart';
